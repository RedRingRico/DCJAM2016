...
#define MEM_Align( pPointer, Alignment ) \
	( ( ( size_t )( pPointer ) + ( Alignment ) - 1 ) & \
		( ~( ( Alignment ) - 1 ) ) )
#define MEM_AlignUp( pPointer, Alignment ) \
	...
	size_t p_Size )
{
	...
	while( pHeader != NULL )
	{
			...
			( TotalSize >= p_Size ) )
		{
			...
			/* TODO:
			 * Make sure that the header size is greater than the total size */
...
{
	PMEMORY_BLOCK_HEADER pHeader, pNext;
...
{
	PMEMORY_BLOCK_HEADER pHeader;
	size_t DesiredSize, PointerSize;
	...
	{
		/* Resize downward, join with the next memory block if possible */
		if( pHeader->pNext->Flags & MEM_BLOCK_FLAG_FREE )
		{
			PMEMORY_BLOCK_HEADER pNewBlock;
	...
	else
	{
		...
		else
		{
			...
			if( FreeTotalSize > FreeOffset )
			{
				...
				MEM_CreateMemoryBlockHeader( pHeader, false, TotalSize, p_NewSize );
				pHeader->pNext = pNewBlock;
			}
			else
			{
...
size_t MEM_GetFreeMemoryBlockSize( PMEMORY_BLOCK p_pMemoryBlock )
{
	...
	while( pBlock )
	{
		if( pBlock->Flags & MEM_BLOCK_FLAG_FREE )
		{
...
size_t MEM_GetUsedMemoryBlockSize( PMEMORY_BLOCK p_pMemoryBlock )
{
	...
	while( pBlock )
	{
		if( ( pBlock->Flags & MEM_BLOCK_FLAG_FREE ) == 0 )
		{
