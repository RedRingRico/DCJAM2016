	...
	const PCAMERA p_pCamera )
{
	...
	FOV2 = cosf( p_pCamera->FieldOfView * 0.5f );
	FOV2 /= sinf( p_pCamera->FieldOfView * 0.5f );
...
void CAM_TransformNonClipPerspective( float *p_pTransformedVertices,
	...
	const PMATRIX4X4 p_pMatrix, const PCAMERA p_pCamera )
{
	register float H, *pDestinationVector, HalfWidth, HalfHeight, RHW;
	register int Index;
	...
	H = tanf( p_pCamera->FieldOfView * 0.5f );
